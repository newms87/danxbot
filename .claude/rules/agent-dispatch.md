...
**Worker death does NOT reap dispatched agents.** Each dispatch's systemd scope (host) / claude PID (docker container) lives in its own cgroup, independent of the worker process tree. `kill`/`SIGTERM` on the worker leaves every live dispatch running — no parent-child relation. The in-worker `reap-orphan-dispatches` job (`src/cron/jobs/reap-orphan-dispatches.ts`, fired by `src/cron/worker-loop.ts` on boot + every 60s) is the only sweeper for scopes the worker leaked through an unclean death, and it only acts on scopes whose `dispatches` row is terminal-or-missing. Practical consequence: when stopping a worker mid-flight (operator restart, code reload), in-progress agents continue to completion + signal via `danxbot_complete` HTTP/DB-fallback; only NEW dispatches are gated by the worker being up. A NEXT worker boot's one-shot reap pass then sweeps any scope leaked during the gap.
...
**In-worker reap safety net (DX-551).** A host crash that kills the worker before `job.stop` runs leaves the scope unit + its cgroup alive. `src/cron/jobs/reap-orphan-dispatches.ts` fires from the in-worker dispatcher (`src/cron/worker-loop.ts`): once on boot BEFORE the HTTP listener accepts dispatches (catches scopes leaked during the previous worker's death), then every 60s on `setInterval` while the worker is alive. It enumerates `danxbot-dispatch-*.scope` units, joins against the `dispatches` table, and `systemctl --user stop`s every scope whose row is terminal-or-missing AND whose scope age clears the 60s race-window guard. Read-only on the DB, kill-only on scopes — no env-variable scan / proc-tree walk, no "kill anything older than X regardless of DB." Per-job `lastRunMs` lives at `<repo>/.danxbot/cron-state.json` so a worker bounce inside the 60s window does not double-fire. The prior `make install-cron` / `src/cron/tick.ts` system-crontab install surface was retired in DX-551; the worker owns the loop now.
...
Agents call `danxbot_complete({status, summary})` (MCP tool, `src/mcp/danxbot-server.ts`). Four agent-visible statuses: `completed`, `failed`, `critical_failure`, `agent_blocked`.

- `completed` / `failed` — normal lifecycle. Dispatch row terminates in that state.
- `critical_failure` — environment-level blocker. Worker writes the per-repo `CRITICAL_FAILURE` flag (halts the poller) and finalizes the row as `failed`.
- `agent_blocked` — agent self-block on the candidate card. Worker stamps `status: Blocked` + `blocked: {reason: summary, timestamp: now}` on the dispatch row's `issue_id` YAML, then finalizes the row as `failed`. REQUIRES `issue_id` on the dispatch row (400 otherwise). The self-block signal lives on the YAML, not the row.

MCP server POSTs `DANXBOT_STOP_URL` (`http://localhost:<worker_port>/api/stop/<dispatchId>`). Worker `job.stop`:
...
| Gating issue-tracker business logic on `trelloSync` (the `reconcileIssue` call site, the picker / scheduler poke chain, `autoSyncTrackedIssue` entry point, `kickPickerOnceAtBoot`, `onReconcileResult`, `onAgentRosterChange`). The only allowed gates are: (a) Trello push step INSIDE reconcile (step 7 — `pushTrelloDiff` at `src/issue/reconcile.ts:614`); (b) Trello inbound module (`src/cron/inbound-fetch.ts`); (c) Trello retry queue (`src/issue-tracker/retry-queue.ts`). | Trello = side system. Issue tracker must run identically with Trello enabled, disabled, or deleted. History: a prior version of `auto-sync.ts` short-circuited the post-dispatch `reconcileIssue` call when `trelloSync=false`, so the `onReconcileResult` poke chain never fired and the picker sat idle after every dispatch on a Trello-disabled repo (queue silently froze). The post-dispatch reconcile MUST fire for every dispatch carrying an `issueId`, regardless of trigger source (Trello / Slack / API / poller) or Trello configuration. Reviewer agents and pipeline gates flag any new Trello-coupled branch outside the three allowed surfaces. |
...
| `git reset --hard`, `git checkout <ref>` (branch / commit switch), `git restore`, `git clean -f` in any dispatch / worktree / recovery code path | Destroys uncommitted agent work irrecoverably. Commit-first is the only default recovery primitive — see `~/web/claude-plugins/dev/skills/git-discipline/SKILL.md` "Never Destroy Work. Ever." The prep skill's WIP recovery commits the residue to the agent's branch; sync uses `fetch + pull --ff-only + rebase`. **One narrow exception** — the prep skill's Step 3 Branch B.2 "orphan-discard window" allows per-file `git checkout HEAD -- <path>` (tracked) + `rm <path>` (untracked) ONLY when ALL of: (a) no YAML scored > 0 against the residue in the 24h timestamp window, (b) residue does not form a coherent change, (c) keeping it would corrupt the next agent's view, (d) agent can state in one sentence why it is junk. Whole-tree `git reset --hard` / `git clean -f` remain banned. |
| `git rebase --abort` in the prep skill's Step 4 branch sync | The prep agent's job is to resolve rebase conflicts in place file by file. Aborting punts the work to no one (no other agent is going to resolve it). Only escalation path is `danxbot_complete({status: "agent_blocked", summary})` when a specific conflict region is genuinely unreconcilable. `git push --force-with-lease` (NOT `--force`) is the post-rebase push primitive — refuses to overwrite remote SHAs the agent has not seen. |
| Calling `dispatch()` with `issueId` set and `dispatchKind === "work"` while expecting the candidate YAML to stay `ToDo` | The auto-flip in `src/dispatch/core.ts` flips the candidate's `status` from `ToDo` → `In Progress` BEFORE `spawnAgent` so the dispatched agent reads its own card already In Progress. Revert on spawn failure rolls the YAML back. Tests / callers that pre-stamp the YAML to a non-ToDo status (already In Progress, Blocked) are no-ops in the flip path (the conditional gates on `status === "ToDo"`). |
