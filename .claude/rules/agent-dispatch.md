...
**Worker death does NOT reap dispatched agents.** Each dispatch's systemd scope (host) / claude PID (docker container) lives in its own cgroup, independent of the worker process tree. `kill`/`SIGTERM` on the worker leaves every live dispatch running — no parent-child relation. The cron reap job (`src/cron/jobs/reap-orphan-dispatches.ts`) is the only sweeper for scopes the worker leaked through an unclean death, and it only acts on scopes whose `dispatches` row is terminal-or-missing. Practical consequence: when stopping a worker mid-flight (operator restart, code reload), in-progress agents continue to completion + signal via `danxbot_complete` HTTP/DB-fallback; only NEW dispatches are gated by the worker being up.
