...
| Calling `runPicker` / `tryMultiAgentDispatch` outside the scheduler's per-repo single-flight mutex (`firePickerWithMutex` / `runWithPickerMutex` in `src/dispatch/scheduler.ts`) | DX-305: three concurrency sources can fire a picker for one repo (onReconcileResult setImmediate, onAgentRosterChange setImmediate, legacy `_poll` direct call). Without the mutex two consecutive macrotasks each invoke `runPicker` against the same `busy`/`assigned` snapshot — both pick the same agent + card and `spawnAgent` runs twice. Adding a fourth source (e.g. a new file-watch poke) without routing through the mutex re-introduces the duplicate-spawn class. Use `firePickerWithMutex(repoName)` for fire-and-forget pokes; use `runWithPickerMutex(repoName, fn)` when the caller needs the `MultiAgentPickResult` back. |
