...
All repo-specific config lives in `repo/` (gitignored, generated by `/setup`):
...
repo/
  ...
  repo.env         # container env vars (optional)
  post-clone.sh    # runs after cloning (optional)
  features.md      # ideator's persistent memory
  docs/
    domains/*.md   # domain knowledge
    schema/*.md    # DB relationships
```

The poller syncs these to their target locations (`.claude/rules/`, `docs/`, `repo-overrides/`) before each Claude spawn. `repo-overrides/` is the synced output — never edit it directly.
