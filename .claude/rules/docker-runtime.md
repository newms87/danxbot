...
- **Runtime commands** depend on the repo's runtime setting in `repo-config.yml`:
...
## Repo Overrides Directory

`repo-overrides/` contains files generated by `/setup` for the connected repo:

- **Compose override files** — Docker Compose files for running the repo's test/dev stack in isolation (e.g., `<name>-compose.yml`)
- **Env files** — Environment variables for the repo's containers (e.g., `<name>.env`)
- **Post-clone hooks** — Scripts named `post-clone-<name>.sh` that run at container startup after cloning (e.g., copying auth files, installing dependencies)

These are gitignored — each installation generates its own based on the connected repo.
