...
2. The ideator will:
   ...
   - Explore the Danxbot and connected repo codebases
...
4. **Self-terminate if ephemeral:** If `DANXBOT_EPHEMERAL=1` is set in the environment (poller-spawned session), remove the lock file and terminate: `rm -f .poller-running && kill $PPID`. If NOT set (interactive session), do nothing — just report results.
