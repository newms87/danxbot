...
2. If the list is empty, report "No cards to process" and self-terminate only if `DANXBOT_EPHEMERAL=1`: `rm -f .poller-running && kill $PPID`
...
5. **Stop after this single card.** Step 9 handles self-termination when `DANXBOT_EPHEMERAL=1` is set.
