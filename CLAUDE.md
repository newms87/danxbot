...
The setup generates `repo-config.yml`, `.env`, and tailored rules files. No manual `.env` editing needed.

## Connected Repo

Repo config is in `repo-config.yml` (source of truth) and `.claude/rules/repo-config.md` (auto-generated by poller). The connected repo is cloned to `repos/<name>/` and is the target for Trello card work.
