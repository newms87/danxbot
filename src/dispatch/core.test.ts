...
describe("dispatch() — DX-105 Phase 2: pre-spawn syncRepoFiles hook", () => {
  // Phase 2 wires `syncRepoFiles(repo)` into `dispatch()` so every
  // dispatch path (poller, /api/launch, Slack) sees fresh workspace
  // state at spawn time even when the cron sync has not ticked since
  // the last source edit. The contract surface is small:
  //
  //   1. The repo's `.danxbot/config/` is read before `resolveWorkspace`.
  //   2. Per-repo rendered files land in the workspace dir BEFORE the
  //      resolver reads its contents — verifiable by editing a source
  //      file post-fixture-copy and asserting the rendered output
  //      reflects the edit at spawn time.
  //
  // Guard against a regression that flips the call order (sync after
  // resolve = stale workspace at spawn).
  const issueWorkerSrc = resolve(
    __dirname,
    "..",
    "inject",
  ...
  beforeEach(() => {
    tmpRepoDir = mkdtempSync(resolve(tmpdir(), "danxbot-dx105-prespawn-"));
    const dest = resolve(tmpRepoDir, ".danxbot", "workspaces", "issue-worker");
    mkdirSync(resolve(tmpRepoDir, ".danxbot", "workspaces"), {
      recursive: true,
    });
    cpSync(issueWorkerSrc, dest, { recursive: true });
    // Minimal `.danxbot/config/` so `syncRepoFiles` clears its initial
    // `!existsSync(danxbotConfigDir)` short-circuit and runs the
    // per-repo render layer.
    const configDir = resolve(tmpRepoDir, ".danxbot", "config");
    mkdirSync(configDir, { recursive: true });
    writeFileSync(
      resolve(configDir, "config.yml"),
      [
        "name: dx105-prespawn",
        "url: https://example.com/dx105.git",
        "runtime: local",
        "language: typescript",
      ].join("\n") + "\n",
    );
    ...
    issueRepo = makeRepoContext({ localPath: tmpRepoDir });
  });
  ...
  it("renders danx-repo-config.md from config.yml into the workspace before spawn", async () => {
    await dispatch({
      repo: issueRepo,
      task: "/danx-next ISS-1",
      workspace: "issue-worker",
      overlay: {},
      apiDispatchMeta: DEFAULT_DISPATCH_META,
    });

    const renderedRule = resolve(
      tmpRepoDir,
      ".danxbot",
      "workspaces",
      "issue-worker",
    ...
    expect(existsSync(renderedRule)).toBe(true);
    const body = readFileSync(renderedRule, "utf-8");
    // Banner from `writeInjectedFile` names the source-of-truth.
    expect(body.startsWith("<!-- AUTO-GENERATED by danxbot from ")).toBe(true);
    expect(body).toContain(".danxbot/config/config.yml");
    // Per-repo rendered content from the config.yml fixture above.
    expect(body).toContain("| Name | `dx105-prespawn` |");
  });

  it("re-renders on source edit between dispatches (no stale workspace cache)", async () => {
    await dispatch({
      repo: issueRepo,
      task: "/danx-next ISS-1",
      workspace: "issue-worker",
    ...
    // Operator edits the source between dispatches.
    writeFileSync(
      resolve(tmpRepoDir, ".danxbot", "config", "config.yml"),
      [
        "name: dx105-prespawn-v2",
        "url: https://example.com/dx105.git",
        "runtime: local",
        "language: typescript",
      ].join("\n") + "\n",
    ...
    const body = readFileSync(renderedRule, "utf-8");
    expect(body).toContain("| Name | `dx105-prespawn-v2` |");
    expect(body).not.toContain("`dx105-prespawn` |");
  });
});
