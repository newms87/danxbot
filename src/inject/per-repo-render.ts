...
function writeRepoConfigRule(
  ...
  writeInjectedFile({
    destAbs: resolve(target.rulesDir, "danx-repo-config.md"),
    source: { content: renderRepoConfigMarkdown(cfg) },
...
function copyRepoConfigDocs(
  ...
  for (const [src, dest] of mappings) {
    ...
    writeInjectedFile({
      destAbs: resolve(target.rulesDir, dest),
      source: { fileAbs: srcPath },
      srcRelToRepoRoot: `.danxbot/config/${src}`,
    });
  }
}
...
function writeIssuePrefixRule(
  ...
  writeInjectedFile({
    destAbs: resolve(target.rulesDir, "danx-issue-prefix.md"),
    source: { content: body },
    srcRelToRepoRoot: ".danxbot/config/config.yml#issue_prefix",
  });
}

/**
 * Step 2c (DX-512 / DX-508 Phase 4): render `danx-effort-policy.md` from
 * `<repo>/.danxbot/settings.json`. Surfaces the operator-tunable
 * effort-assignment prompt + the operator-configured 7-row level ladder
 * as cwd-resolvable markdown the `issue-card-workflow`, `danx-triage-card`,
 * and `danx-next` plugin skills point at when picking `effort_level` for
 * a card. Re-rendered every tick — operator edits in the dashboard
 * propagate without a worker restart. The DX-509 settings readers
 * (`getEffortLevels`, `getEffortAssignmentPrompt`) own the fallback path
 * to built-in defaults; this helper never sees a missing-settings error.
 *
 * Relies on `getEffortLevels`' length-7 canonical-order invariant
 * (`settings-file.ts` `normalizeEffortLevels`) — every row lands in the
 * rendered table verbatim. A future refactor that relaxes the invariant
 * would silently change the table shape; pin the invariant in
 * `settings-file.test.ts` before touching the reader.
 */
function writeEffortPolicyRule(
  repo: RepoContext,
  target: InjectTarget,
): void {
  const levels = getEffortLevels(repo.localPath);
  const prompt = getEffortAssignmentPrompt(repo.localPath);
  const tableRows = levels
    .map((row) => `| ${row.name} | ${row.model} | ${row.effort} |`)
    .join("\n");
  const body =
    `# Effort Policy\n` +
    `\n` +
    ...
    `${prompt}\n`;
  writeInjectedFile({
    destAbs: resolve(target.rulesDir, "danx-effort-policy.md"),
    source: { content: body },
    srcRelToRepoRoot: ".danxbot/settings.json#effortLevels+effortAssignmentPrompt",
  });
}
...
function copyRepoToolsDoc(
  ...
  writeInjectedFile({
    destAbs: resolve(target.rulesDir, "danx-tools.md"),
    source: { fileAbs: src },
    srcRelToRepoRoot: ".danxbot/config/tools.md",
  });
}
...
function copyRepoToolScripts(
  ...
  for (const file of readdirSync(src)) {
    ...
    writeInjectedFile({
      destAbs: dest,
      source: { fileAbs: resolve(src, file) },
      srcRelToRepoRoot: `.danxbot/config/tools/${file}`,
    });
...
export function renderPerRepoFilesIntoWorkspaces(
  ...
  for (const name of names) {
    ...
    writeEffortPolicyRule(repo, target);
...
export function copyComposeOverride(
  ...
  writeInjectedFile({
    destAbs: resolve(overridesDir, `${cfgName}-compose.yml`),
    source: { fileAbs: src },
    srcRelToRepoRoot: ".danxbot/config/compose.yml",
  });
}
...
export function copyRepoDocs(danxbotConfigDir: string): void {
  ...
  for (const subdir of ["domains", "schema"]) {
    ...
    for (const file of readdirSync(srcDir)) {
      writeInjectedFile({
        destAbs: resolve(destDir, file),
        source: { fileAbs: resolve(srcDir, file) },
        srcRelToRepoRoot: `.danxbot/config/docs/${subdir}/${file}`,
      });
    }
  }
}
...
/**
 * Step 9 (DX-105): features.md mirror — runs every tick. Pre-DX-105 the
 * helper was `copyFeaturesOnce` with an `existsSync(dest)` bail; that
 * was a leftover from the era when the ideator wrote ITS edits into the
 * generated copy and the bail protected those edits. Today the
 * source-of-truth is `<repo>/.danxbot/features.md` (operator + ideator
 * write THERE; the generated copy under `docs/features.md` is derived).
 * Drop the bail — restore the DRY contract — and run every tick.
 */
export function copyFeatures(danxbotConfigDir: string): void {
  ...
  writeInjectedFile({
    destAbs: resolve(projectRoot, "docs", "features.md"),
    source: { fileAbs: src },
    srcRelToRepoRoot: ".danxbot/features.md",
  });
}
