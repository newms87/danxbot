 ...
 * Open-issue status enum.
 *
 * `Blocked` (formerly `Needs Help`) is the non-dispatchable parking status
 * for cards that cannot make progress on their own work — the card itself
 * is blocked. A human (or a subsequent agent run) must clear the block.
 * Distinct from:
 *  - `Needs Approval` — the agent COULD do the work but wants a sanity
 *    check on the chosen direction before proceeding (architectural risk,
 *    cross-cutting scope). Conflating the two loses signal for operators
 *    triaging the parked-card list.
...
export interface Issue {
   ...
   * Needs Approval buckets, `priority` DESC tiebreaks ICE-equal cards.
...
export interface ManagedLabels {
  type: IssueType;
  /**
   * `true` ⟺ status === "Blocked" (formerly `needsHelp`). Mapped to the
   * tracker's "Blocked" label. The boolean is derived from `status`, NOT
   * from `Issue.blocked` — the field is the reason cache, the status is
   * the index lookup. Worker enforces the field/status invariant.
   */
  blocked: boolean;
