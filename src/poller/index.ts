...
const repoConfigRule = resolve(projectRoot, ".claude/rules/repo-config.md");
const repoConfigYml = resolve(projectRoot, "repo-config.yml");
...
/**
 * Write the repo config rule file so the spawned Claude agent always has
 * the correct repo commands and paths. Generated from repo-config.yml.
 */
function writeRepoConfigRule(): void {
  if (!existsSync(repoConfigYml)) return;
...
This file is generated by the poller from \`repo-config.yml\` before each run.
...
`;
  ...
  writeFileSync(repoConfigRule, content);
}
