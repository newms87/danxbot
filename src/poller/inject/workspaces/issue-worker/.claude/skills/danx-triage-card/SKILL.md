...
2. Decide per `status` (Review / Needs Help / Blocked) — apply the per-status decision tree below.
...
`blocked` field FIRST, then by `status`:

| YAML state | Path |
|---|---|
| `blocked != null` (regardless of `status`) | **Blocked** — re-check `blocked.by[]` |
| `blocked == null` AND `status === "Review"` | **Review** — ICE-score |
| `blocked == null` AND `status === "Needs Help"` | **Needs Help** — Hard Gate audit |

`Blocked` is NOT a top-level `status` value. The schema's allowed statuses
are `Review`, `ToDo`, `In Progress`, `Needs Help`, `Needs Approval`, `Done`,
`Cancelled`. A "blocked" card has `blocked: {reason, timestamp, by[]}`
populated; the worker mechanically forces `status: ToDo` whenever `blocked`
is non-null. A card with `status: ToDo` AND `blocked != null` IS a Blocked
card and routes to the Blocked path; a card with `status: ToDo` AND
`blocked == null` is dispatchable work and OUT OF SCOPE for triage (the
...
other shape (`status` ∈ `{ToDo, In Progress, Done, Cancelled, Needs Approval}` AND `blocked == null`) the poller refuses to dispatch — if you somehow receive one, fail loud with `danxbot_complete({status: "failed", summary: "..."})` and do NOT mutate the YAML.
...
| `Needs Help` | 3h | A human is supposed to act; checking back every 3h surfaces stalled blockers fast. |
| `Blocked` | 1h | Blocking cards may flip to terminal at any moment — re-check on the same hour. |
...
ICE is only persisted on **Review** decisions (`Keep` / `Cancel` / `Approve`). For Needs Help and Blocked, leave `triage.ice` at zeros (the cards already have a status; ICE is for prioritising the dispatch queue, which only applies to ToDo).

## reassess_hint contract (Needs Help / Blocked only)
...
| "Re-check `blocked.by[]` — ISS-42 still in progress?" | "Still blocked" |

Hint is REQUIRED on every Needs Help and Blocked save (whether confirming, demoting, or unblocking). Empty `triage.reassess_hint` is a save error.
...
### Status = `Needs Help` — Hard Gate audit

A card sits in Needs Help because some prior agent claimed a human-only blocker. **Audit that claim** before re-confirming. Reuse the misclassification logic from `claude-plugins/issues/skills/unblock/SKILL.md`:

1. Read the most recent `author: danxbot` comment containing a `## Needs Help` / "Operator must do" section. That's the contract.
...
|---|---|---|
| **Every step is locally executable** — wrongly punted | **Demote** to ToDo. Append a comment naming the misclassification (which steps were local-runnable). | `status: ToDo`, `triage.last_status: Demote`, `triage.last_explain: "<one sentence — what was misclassified>"`, `triage.reassess_hint: ""` (cleared — card is no longer parked). The next worker dispatch will pick it up and DO the local work; this triage agent does NOT execute the steps itself. |
| **At least one step is genuinely human-only** | **Confirm** Needs Help. Write `triage.reassess_hint` — the one fast check that decides whether to re-confirm next time. | `status: Needs Help` (unchanged), `triage.last_status: Confirm-Block`, `triage.last_explain: "<one sentence — which human-only step gates the card>"`, `triage.reassess_hint: "<≤120 chars, action-shaped>"`. |
...
`triage.expires_at = now + 3h` on every Needs Help save (Confirm, Demote, or Mixed). Stamp `now + 3h` even on Demote — if the demoted card bounces back to Needs Help later, the cadence is the right floor.
...
### Status = `Blocked` — re-check `blocked.by[]`

A card with `blocked != null` is parked waiting on other in-flight cards. Your job is to re-check whether every blocker has reached a terminal status (Done or Cancelled).

Procedure:

1. Read `blocked.by[]` from the YAML. For each `<ISS-N>`:
...
| Audit outcome | Action | YAML changes |
|---|---|---|
| **Every blocker is terminal** | **Unblock**. Set `blocked: null` (the worker mechanically forces `status: ToDo` on save when `blocked` is null and the prior status was the worker-managed `ToDo` — see `issue-card-workflow` "Blocked vs Needs Help"). | `blocked: null`, `triage.last_status: Unblock`, `triage.last_explain: "<one sentence — every blocker reached terminal status>"`, `triage.reassess_hint: ""` (cleared — card is dispatchable). |
| **At least one blocker is non-terminal** | **Confirm-Block**. Update `triage.reassess_hint` to name which blockers are still pending. | `blocked` unchanged, `status` unchanged (worker enforces ToDo), `triage.last_status: Confirm-Block`, `triage.last_explain: "<one sentence — naming the still-pending blockers>"`, `triage.reassess_hint: "<≤120 chars — e.g. 'Re-check ISS-91, ISS-92 — still in progress as of <iso>'>"`. |

`triage.expires_at = now + 1h` on every Blocked save. The 1h cadence is intentionally short — a phase sibling can move from In Progress to Done at any minute, and we want the dependent card dispatched as soon as possible.
...
A card is **out of scope** ONLY when BOTH conditions hold:

1. `blocked == null` (no blocker record), AND
2. `status` is one of: `ToDo`, `In Progress`, `Done`, `Cancelled`, `Needs Approval`.

| `status` (with `blocked == null`) | Action |
|---|---|
...
**A card with `blocked != null` is NEVER out of scope** — even if its `status` is `ToDo` (the worker forces `ToDo` on every blocked card). Always route blocked cards to the Blocked path. Re-read the in-scope table at the top of "Per-status decision trees" if you find yourself looking at `status: ToDo` and considering refusal — the FIRST check is `blocked != null`, not `status`.
...
4. `triage.reassess_hint` — required for Needs Help (Confirm) and Blocked (Confirm-Block). Cleared (`""`) on Demote and Unblock. Empty on Review.
...
- `text:` markdown body — uses the `comment-style` skill (`claude-plugins/issue-worker/skills/comment-style/SKILL.md`):
  ...
  - `**Reassess hint:** <reassess_hint>` — Needs Help **Confirm** and Blocked **Confirm-Block** ONLY. **DO NOT include this line for Review (any decision), Needs Help / Demote, or Blocked / Unblock** (`triage.reassess_hint` is `""` for those — printing `**Reassess hint:**` with no value is a half-line of garbage in the dashboard).
...
2. After dispatch finishes (`status: completed`), re-read the YAML:
   ...
   - For Needs Help / Confirm and Blocked / Confirm-Block: `triage.reassess_hint` is non-empty AND ≤120 characters.
   - For Needs Help / Demote and Blocked / Unblock: `triage.reassess_hint` is `""` (cleared — card is no longer parked).
...
- For Needs Help cards, the audit is read-only — if the misclassification is "every step locally executable," you Demote and let the next worker dispatch DO the steps. You never run the steps yourself.
