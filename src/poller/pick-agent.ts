...
export function buildReconcileTaskBody(
  ...
  return [
    ...
    "",
    "1. **Inspect each card.** For every id listed above, `Read .danxbot/issues/open/<id>.yml` (fall back to `closed/<id>.yml`) and review the YAML's description, AC, recent comments, and any prior dispatch history. Inspect your worktree state on your agent branch (`git log --oneline -20`, `git status`) for evidence of which card you were actively working.",
