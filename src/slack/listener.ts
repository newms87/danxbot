  ...
  if (!client) return;
    ...
    try {
      ...
      if (routerResult.needsAgent) {
        ...
        // Every dispatch follows the same flow — placeholder + heartbeat +
        // retries — regardless of complexity. Complexity is passed through
        // to runAgent (driving model/budget selection), but it never changes
        // the Slack-side presentation. See the handler-level comment above
        // for why: if we dispatched the agent, the user gets the full
        // feedback experience, full stop.
