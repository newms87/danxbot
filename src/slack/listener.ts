...
async function launchSlackDispatch(
  ...
  // user's message. `dispatch` returns as soon as the claude process is
  // spawned; `onComplete` fires when it reaches a terminal status.
  // Errors thrown before spawn (workspace resolve / MCP init) reject the
  // outer promise and surface to the caller as operational errors. The
  // slack-worker workspace declares its own allowed-tools
  // (`Read`/`Glob`/`Grep`/`Bash` + the two `mcp__danxbot__danxbot_slack_*`
  // tools) and gates on `settings.slack.enabled ≠ false`; the overlay
  // below supplies the per-dispatch placeholders the workspace requires.
  // `apiDispatchMeta.trigger: "slack"` is persisted on the dispatch row
