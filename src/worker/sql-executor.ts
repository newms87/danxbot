...
// `g` flag drives the iterative `.exec` loop in extractSqlBlocks. We
// build a fresh RegExp instance per call so two concurrent callers
// can't trample each other's `lastIndex`.
const SQL_BLOCK_REGEX_SOURCE = "```sql:execute\\n([\\s\\S]*?)\\n```";
const MAX_ROWS = 50;
...
export interface SqlBlock {
  fullMatch: string;
  query: string;
  /** Byte offset of the first character of the block in the source text. */
  start: number;
...
/**
 * Extract all ```sql:execute blocks from response text. Each block
 * carries its byte offsets so callers can splice replacements back in
...
export function extractSqlBlocks(text: string): SqlBlock[] {
  ...
  const regex = new RegExp(SQL_BLOCK_REGEX_SOURCE, "g");
...
export async function processResponseWithAttachments(text: string): Promise<ProcessedResponse> {
  ...
  // Splice replacements back in from last to first so earlier offsets
  // stay valid as we shorten or lengthen the tail. `start`/`end` came
  // from the same regex pass that produced `blocks` — no second walk.
  let result = text;
  for (let i = blocks.length - 1; i >= 0; i--) {
    const block = blocks[i];
    result = result.slice(0, block.start) + replacements[i] + result.slice(block.end);
  }
